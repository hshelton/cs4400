 ...
 * u0658884
 * CADE login : hshelton
 * Partner: Vince Oveson CADE login: oveson
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline) 
{
    ...
    if(builtIn == 0)
    {
        ...
        { 
            unix_error("error adding job in eval");
 ...
 * it immediately.  
 * Given a char ** (basically a string array) of command line args, 
 * determine if it represents a built in command, if so return, execute it and return 1, else
 * return 0
 * We're always checking argv[0] since the command should come first
 * The built in commands supported are: quit , jobs , bg <job> , fg <job>
 */
int builtin_cmd(char **argv) 
{
	//if quit was typed just exit
	...
	else if (strcmp(argv[0], "jobs") ==0)
	{
		...
		// call list jobs to print out each job in the array of job_t structs
		listjobs(jobs);
		return 1;
	}
...
void do_bgfg(char **argv) 
{
       	...
       	if(!argv[1])
	{
	   ...
	   return;
        ...
        if (x == '%') //if it starts with % then it's a job id
		{
			argv1++; // strip of the % from argv1
			int jobId = atoi(argv1);
		...
		else //looking it up by process id
		{
		...
		//at this point we have a job to send SIGCONT to
	       if(kill(-(job->pid), SIGCONT) < 0) unix_error("Kill error in bg_fg");
...
void waitfg(pid_t pid)
{
	...
	{
		//changed this
		if(sleep(1) < 0); /* checking system call */
	}
...
void sigint_handler(int sig) 
{
	...
	{
		if(kill(-processID, SIGINT) <0) unix_error("Error occured while handling SIGINT");
	}
